<?php
	...
	$foo = array(
		"t" => array("name" => "tweets", "required" => 1, "help" => "The path to your archived Tweets, as generated by the backup_tweets.py script."),
		"o" => array("name" => "output", "required" => 1, "help" => "The path for the final PDF file you're creating."),
		"u" => array("name" => "username", "required" => 0, "help" => "The username of the person whose Tweets you're creating a book of. Unfortunately this information is not store in individual Tweets so you need to define it explicitly if you want it to appear in the book."),
		"y" => array("name" => "year", "required" => 0, "help" => "The year of Tweets you're created a book of. Defaults to the current year."),
	);

	$opts = cli_getopts($foo);

	$src = $opts['t'];
	$out = $opts['o'];

	$year = (isset($opts['y'])) ? $opts['y'] : date('Y', time());
	$user = $opts['u'];
